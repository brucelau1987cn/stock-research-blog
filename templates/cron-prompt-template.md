...
- 跟踪段统一使用 H3 格式：`### 跟踪 YYYY-MM-DD HH:MM BJT`（最新在上），结构必须精简、专业，严格按以下格式输出（不要有冗余字段）：
  **触发原因：** ...
  
  **📊 盘面数据快照：**
  - **收盘价 / 盘中最新价**：**X.XX 元**（±X.XX%，iWencai 口径）
  - **资金与动能**：量比 **X.XXx**，换手率 **X.XX%**，主力大单净流入 **X.XX**（散户净流向）
  - **收盘/盘中获利盘**：**X.XX%**（描述筹码趋势及形态）
  
  **🛡 当前对峙边界：**
  - 🔼 **直属攻击线**（上方阻力）：**X.XX 元**（距 X.XX%）
  - 🔽 **直属防守线**（下方支撑）：**X.XX 元**（距 X.XX%）
  - 🚨 **风控/止损底线**：**X.XX 元**（距 X.XX%，说明风控状态）
  
  **🎯 条件执行计划 (If-Then Plan)：**
  - **IF 向上**：若日内放量（量比 > 1.5x）站稳 攻击线 元 → 动作建议
  - **IF 向下**：若收盘实质性跌破 防守线 元 → 动作建议
- 你的责任是只输出正确格式的 H3，sidebar 自动接管
...
**3b. 更新 frontmatter（每次同步）：**
- **每次运行都必须更新 frontmatter `title` 字段**。格式为：'{{CODE}} {{NAME}}：MM/DD [上午/收盘] X.XX 元 (涨跌幅%) [一句话最新状态]'。
- 同步 `updatedDate`，仅表示文章维护时间；`pubDate` 不动。
- `description` = "最新结论：<一句话重写当前框架>"（保持 80–120 字内）。
- 同步完整 `decision`，至少包括 `status/currentPrice/previousClose/changePct/resistance/support/invalidation/action/changeSummary`。
- `decision.market` 必须是 `CN/US/HK`；`sessionDate` 是行情所属交易日；`dataAsOf` 是带时区偏移的行情截止时间。不得用 `updatedDate` 代替行情时间。
- `invalidation.direction` 必须是 `below/above`；`state` 必须是 `pending/near/triggered`，且必须与现价和失效位方向一致。
- 如果行情交易日或时间无法核实，停止编辑，不能把旧行情伪装成新快照。
...
### Step 5: 推送格式（命中时，或下午盘后简报）
```
📊 {{NAME}} {{CODE_PLAIN}} 跟踪 (操盘日志)

⏰ YYYY-MM-DD HH:MM BJT
💰 最新价：X.XX 元（±X.XX%）
📈 动能资金：量比 X.XXx · 换手率 X.XX% · 主力大单 X.XX
🧩 获利盘：X.XX%

...
**其他情况静默**（不推送，节省 tokens）。但**每次运行都把最新价/量能写到本地状态文件** `/root/.hermes/state/{{SLUG}}_tracker.json`（覆盖前次内容），便于后续推理。
...
### Step 2: 判定是否触发推送
按上面 6 条规则过一遍。任一命中 → 推送 + 更新博客。未命中 → 静默，只更新 state 文件。

### Step 3: 触发时同步到博客（顶部插入 H3 + 全文修订 + 5 交易日清理）
...
git commit -m "[{{SLUG}}-tracker] YYYY-MM-DD HH:MM 触发原因简述 + 全文修订 (结论先行+近1月+...) [+ archive <删除了的日期>]"
